...
class InterceptorRule {
   ...
   * 拦截规则
   *
   * @param {function} shouldCancel 是否应该取消请求
  ...
  rules.forEach(rule => {
    ...
    let onFulfilled = async function(config) {
      ...
      if (typeof shouldCancel === 'function') {
        let reason = await shouldCancel(config)
      ...
      if (requestParams) {
        ...
        requestParams.forEach((v, k) =>
          typeof v === 'function' ? chain.push([k, v]) : (params[k] = v)
        )
        if (chain.length) {
          let promise = Promise.resolve()
          while (chain.length) {
            let [k, f] = chain.shift()
            promise = promise.then(f).then(v => (params[k] = v))
          }
        }
      }
      // 添加请求头
      if (requestHeaders) {
        ...
        requestHeaders.forEach((v, k) =>
          typeof v === 'function' ? chain.push([k, v]) : (headers[k] = v)
        )
        if (chain.length) {
          let promise = Promise.resolve()
          while (chain.length) {
            let [k, f] = chain.shift()
            promise = promise.then(f).then(v => (headers[k] = v))
          }
        }
      }

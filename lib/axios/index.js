      ...
      if (config) {
        ...
        rules.forEach(rule => {
          ...
          if (rule.requestHeaders) {
            rule.requestHeaders.forEach((v, k) => {
              config.headers[k] = v
            })
          }
        })
...
function onResultFault(faultRules, res) {
  /* 处理响应失败 */
  if (Array.isArray(faultRules) && faultRules.length) {
    let failPromises = faultRules.map(rule => rule.onResultFault(res, rule))
    if (failPromises.length) {
      return Promise.all(failPromises).then(() => Promise.reject(res.data))
    }
  }
}
